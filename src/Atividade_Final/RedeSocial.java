...
public class RedeSocial {
	...
	static ArrayList<Usuario> usuariosRede = new ArrayList<Usuario>();
	static ArrayList<Postagem> todasPostagens = new ArrayList<Postagem>();
	...
	public static void main(String[] args) {
		...
		fazerPostagem(1, dataAtual(), horaAtual(), "Minha viagem está sendo TOPPPP !", todasPostagens);
	...
	static void telaCadastro() {
		...
		for(Usuario logins : usuariosRede) {
			if(logins.login.toLowerCase().equals(login.toLowerCase())) {
				verificaLogin = true;
			}
		}
	...
	static void telaLogin() {
		...
		for(int i = 0; i < usuariosRede.size(); i++) {
			if(usuariosRede.get(i).login.toLowerCase().equals(login.toLowerCase()) && usuariosRede.get(i).senha.equals(senha)) {
				verificaLogin = true;
				idUsuario = i;
				break;
			}
	...
	static void telaMenu(int idUsuario) {
		limpaConsole();
		System.out.println("\t*********************************************************************************************************");
	...
	static void telaTimeline(int idUsuario) {
		...
		if(qtdPostProprios == 0) {
			...
			for(int i = todasPostagens.size()-1; i >= 0; i--) {
				if(todasPostagens.get(i).idUsuario == idUsuario) {
					idPostagens.add(i);
					System.out.println("\t*\t\t\t\t\t\t\t\t\t\t     " + todasPostagens.get(i).data + " - " + todasPostagens.get(i).hora );
					System.out.println("\t* ("+(i+1)+") Você postou:\n\t*");
					imprimeConteudoPost(todasPostagens.get(i).conteudo);
	...
	static void telaFeed(int idUsuario) {
		...
		if(qtdPostDeSeguindo == 0) {
			...
			for(int i = todasPostagens.size()-1; i >= 0; i--) {
				for(int idSeguindo : usuariosRede.get(idUsuario).seguindo) {
					if(todasPostagens.get(i).idUsuario == idSeguindo) {
						idPostagens.add(i);
						System.out.println("\t*\t\t\t\t\t\t\t\t\t\t     " + todasPostagens.get(i).data + " - " + todasPostagens.get(i).hora );
						System.out.println("\t* ("+(i+1)+") " + usuariosRede.get(idSeguindo).nome + " postou:\n\t*");
						imprimeConteudoPost(todasPostagens.get(i).conteudo);
						System.out.println("\t*\t\t\t\t\t\t\t\t\t\t\t" + todasPostagens.get(i).comentarios.size() + " comentários");
						System.out.println("\t*********************************************************************************************************");
					}
	...
	static void verPostagem(int idUsuario, int idPostagem) {
		limpaConsole();
		System.out.println("\t*********************************************************************************************************");
		System.out.print("\t********\t       Bem-Vindo(a) " + usuariosRede.get(idUsuario).nome);
		System.out.print("\t\t\t -- " + usuariosRede.get(idUsuario).seguidores.size() + " Seguidores");
		...
		if(todasPostagens.get(idPostagem).idUsuario == idUsuario) {
			...
			System.out.println("\t* ("+(idPostagem+1)+") " + usuariosRede.get(todasPostagens.get(idPostagem).idUsuario).nome + " postou:\n\t*");
		}
		...
		if(todasPostagens.get(idPostagem).comentarios.size() == 0) {
			...
			for(int i = 0; i <= todasPostagens.get(idPostagem).comentarios.size()-1; i++) {
				System.out.println("\t*\t\t\t\t\t\t\t\t\t\t     " + todasPostagens.get(idPostagem).data + " - " + todasPostagens.get(idPostagem).hora );
				if(todasPostagens.get(idPostagem).comentarios.get(i).idUsuario == idUsuario) {
					System.out.println("\t* Você comentou:\n\t*");
				}
				else {
					System.out.println("\t* " + usuariosRede.get(todasPostagens.get(idPostagem).comentarios.get(i).idUsuario).nome + " comentou:\n\t*");
				}
				imprimeConteudoPost(todasPostagens.get(idPostagem).comentarios.get(i).conteudo);
				System.out.println("\t*" );
				System.out.println("\t*********************************************************************************************************");
			}
		}
	...
	static void telaComentario(int idUsuario, int idPostagem) {
	...
	static void telaBuscaUsuario(int idUsuario) {
		String nomeBusca;
		...
		if(nomeBusca.trim().length() == 0) {
			...
			for(int i = 0; i < usuariosRede.size(); i++) {
				if(!(i == idUsuario)) {
					if(usuariosRede.get(i).nome.toLowerCase().contains(nomeBusca.toLowerCase())) {
						idResultados.add(i);
					}
				}
			}
	...
	static void telaResultadoBusca(int idUsuario, ArrayList<Integer> resultadoBusca) {
		...
		for(int i = 0; i < resultadoBusca.size(); i++) {
			System.out.println("\t*     (" + (resultadoBusca.get(i)+1) + ")  " + usuariosRede.get(resultadoBusca.get(i)).nome);
	...
	static void telaSeguidores(int idUsuario) {
		String idUsuarioEscolhido;
		...
		if(usuariosRede.get(idUsuario).seguidores.size() == 0) {
			...
			for(int i = 0; i < usuariosRede.get(idUsuario).seguidores.size(); i++) {
				System.out.println("\t*     (" + (usuariosRede.get(idUsuario).seguidores.get(i)+1) + ")  " + usuariosRede.get(usuariosRede.get(idUsuario).seguidores.get(i)).nome);
	...
	static void telaSeguindo(int idUsuario) {
		String idUsuarioEscolhido;
		...
		if(usuariosRede.get(idUsuario).seguindo.size() == 0) {
			...
			for(int i = 0; i < usuariosRede.get(idUsuario).seguindo.size(); i++) {
				System.out.println("\t*     (" + (usuariosRede.get(idUsuario).seguindo.get(i)+1) + ")  " + usuariosRede.get(usuariosRede.get(idUsuario).seguindo.get(i)).nome);
	...
	static void imprimeConteudoPost(String conteudo) {
	...
	static void fazerPostagem(int idUsuarioPost, String dataPost, String horaPost, String conteudoPost, ArrayList<Postagem> todosPosts) {
		todosPosts.add(new Postagem(idUsuarioPost, dataPost, horaPost, conteudoPost));
	}
	
	static void comentarPostagem(int idUsuarioComentario, int idPostagem, String dataPost, String horaPost, String conteudoComentario, ArrayList<Postagem> todosPosts) {
